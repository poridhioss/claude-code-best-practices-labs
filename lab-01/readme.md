...
A coding assistant is more than a tool that writes code. It is a system that uses a language model to tackle complex programming tasks — reading files, understanding errors, editing code, running tests — by combining text generation with real world actions.
...
<!-- diagram: assistant architecture — task flows into assistant box containing language model + tools, with a loop of gather context → formulate a plan → take action → iterate -->
...
Here is the fundamental challenge: language models can only process text and return text. If you ask a standalone LLM to read a file, it will tell you it can not.
...
<!-- diagram: tool use sequence — coding assistant on the left, language model on the right, with arrows showing request/response flow for reading a file -->
...
- **Knowing what to read** — Claude navigates codebases without needing a pre built index. It uses glob patterns to find files, grep to search contents, and reads only what is relevant. This means your entire codebase does not need to be sent to a server.
...
Watch the terminal — you will see Claude make a **Read** tool call, receive the file contents, and then answer based on what it read. The model did not already know what is in that file. It requested the action, the CLI executed it, and the contents came back.

<!-- screenshot: Claude reading README.md — showing the Read tool call and response -->
...
<!-- screenshot: Claude using Glob then Read to find and examine weather.js and formatter.js -->
...
When you give Claude Code a task, it does not just respond once. It enters a loop:
...
That is 8 tool calls from a single prompt. Claude decided each step based on the previous result. It discovered step 4 only after seeing the error at step 3. Each observation shapes the next action.
...
Do not interrupt — let Claude work. Watch the terminal and count the tool calls. Claude will use Glob to scan directories, Read to examine files it finds interesting, maybe Grep to search for patterns. Each step is a decision based on what it learned in the previous step.

<!-- screenshot: Claude exploring the project — showing sequential Glob/Read calls as it discovers and reads files -->
...
When Claude needs information from multiple independent sources, it does not read them one at a time. It batches independent operations into parallel calls.
...
Here is the difference in practice. When you ask vaguely, Claude has to search first, then read what it finds — that is sequential work across multiple turns. When you are specific about which files to look at, Claude reads them all simultaneously in a single turn.
...
This is not a minor optimization. Over a 20 step task, the difference between "Claude searches then reads" and "Claude reads directly" can save significant context and time. In a lab environment with limited tokens, this matters even more.
...
Watch the terminal — you will see Claude read both files at the same time (two Read calls with no gap between them). It knows both file paths from your prompt, so it reads them in parallel instead of one after the other.

<!-- screenshot: two simultaneous Read calls for weather.js and formatter.js -->
...
| **Sonnet 4.5** | $3 / $15 per 1M tokens | Daily coding, feature implementation, debugging |
| **Opus 4.6** | $5 / $25 per 1M tokens | Architecture decisions, complex reasoning, multi file refactors |

> Models with extended 1M token context are also available at higher pricing.

The practical pattern: **Sonnet** is the default and covers most daily work. Switch to **Opus** when Claude is making wrong architectural decisions or missing edge cases. Drop to **Haiku** for repetitive tasks like formatting, renaming, or generating boilerplate.
...
This prevents the most expensive mistake in agentic coding: Claude going down the wrong path, burning context, and needing to start over. Without plan mode, Claude might start implementing a feature, realize halfway through that it chose the wrong approach, and now your context is filled with work you are about to undo. With plan mode, Claude explores first, presents its approach, and you can redirect before any code is written.
...
Watch how Claude explores the codebase — reading the route files, the coordinates utility, the tests — but does not write any code. It presents a plan instead. This is the explore first approach that saves context on complex tasks.

<!-- screenshot: Claude in plan mode — exploring files and presenting a plan without making changes -->
